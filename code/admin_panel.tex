...
\textbf{Database} section defines a category for each table of the \textsl{dwarfs4MOSAIC} platform, allowing to manage records as required.
	
\insertFigure[0.75]{admin/overview-database}{Database administration.}
	

When selecting a category in the administration panel, a list of existing objects of that type is displayed, allowing you to view, filter, edit, or delete them as needed.

\insertFigure{admin/list-instruments}{List of instruments.}


All categories in the administration panel have the same basic actions.

% CSV import functionality temporarily disabled.
% The explanation is preserved for potential future reactivation.
...
The administration panel provides filtering tools to facilitate the search and management of objects within each category.

When selecting a category, a filter sidebar is displayed on the right-hand side of the list view. This panel allows users to narrow down the displayed results based on one or more field values specific to the selected model.

Filters are applied dynamically, updating the list of objects without modifying the database. Multiple filters can be combined to refine the search further and quickly locate specific entries.

\insertFigure{admin/filter-objects.jpg}{Filter objects.}
...
\begin{description}
	...
	\item[Website:] link to an external astronomical database or information page for the object.
	\item[Right Ascension:] right ascension of the target, expressed in hours, minutes, and seconds.
	\item[Declination:] declination of the target, expressed in degrees, arcminutes, and arcseconds, with positive values for northern and negative for southern declinations.
	\item[Magnitude:] apparent magnitude of the object in the $r$ band (AB system). 
	\item[Redshift:] redshift of the object with error.
	\item[Size:] apparent angular size of the object, given in arcseconds.
\end{description}

\insertFigure[0.70]{admin/add-target-2}{Target general information.}
